...
static rt_s zz_execute_statement(rt_char8 *sql, struct da_statement *statement, rt_b ignore_errors, rt_un *row_count)
{
	...
	if (RT_UNLIKELY(!rt_encoding_decode(sql, rt_char8_get_size(sql), RT_ENCODING_SYSTEM_DEFAULT, buffer, RT_CHAR_HALF_BIG_STRING_SIZE, RT_NULL, RT_NULL, &output, &buffer_size, RT_NULL))) goto end;
...
static rt_s zz_test_execute_with_statement(struct da_statement *statement, struct da_connection *connection, enum da_database_type database_type)
{
	...
	case DA_DATABASE_TYPE_MSSQL:
	case DA_DATABASE_TYPE_POSTGRES:
		if (RT_UNLIKELY(!zz_execute_statement("drop table if exists RDA_TESTS_TABLE", statement, RT_FALSE, RT_NULL))) goto end;
		if (RT_UNLIKELY(!zz_execute_statement("create table RDA_TESTS_TABLE (VAL1 varchar(200), VAL2 varchar(200), VAL3 numeric(15))", statement, RT_FALSE, RT_NULL))) goto end;
		break;
	case DA_DATABASE_TYPE_ORACLE:
		if (RT_UNLIKELY(!zz_execute_statement("drop table RDA_TESTS_TABLE", statement, RT_TRUE, RT_NULL))) goto end;
		if (RT_UNLIKELY(!zz_execute_statement("create table RDA_TESTS_TABLE (VAL1 varchar2(200), VAL2 varchar2(200), VAL3 number(15))", statement, RT_FALSE, RT_NULL))) goto end;
		break;
	}

	if (RT_UNLIKELY(!connection->commit(connection))) goto end;

	if (RT_UNLIKELY(!zz_execute_statement("insert into RDA_TESTS_TABLE values ('1', '', 1)", statement, RT_FALSE, RT_NULL))) goto end;
	if (RT_UNLIKELY(!zz_execute_statement("insert into RDA_TESTS_TABLE values ('2', null, 2)", statement, RT_FALSE, RT_NULL))) goto end;
	if (RT_UNLIKELY(!zz_execute_statement("insert into RDA_TESTS_TABLE values ('2', null, 2)", statement, RT_FALSE, RT_NULL))) goto end;
	...
	if (RT_UNLIKELY(!connection->rollback(connection))) goto end;

	if (RT_UNLIKELY(!zz_execute_statement("update RDA_TESTS_TABLE set VAL1 = '3' where VAL1 = '2'", statement, RT_FALSE, &row_count))) goto end;
	if (RT_UNLIKELY(row_count != 2)) goto end;

	if (RT_UNLIKELY(!connection->commit(connection))) goto end;

	if (RT_UNLIKELY(!zz_execute_statement("insert into RDA_TESTS_TABLE values ('333', null, 333)", statement, RT_FALSE, RT_NULL))) goto end;
...
static rt_s zz_test_execute_with_connection(struct da_connection *connection, enum da_database_type database_type)
{
	...
	if (statement_created) {
		if (RT_UNLIKELY(!statement.free(&statement)))
			ret = RT_FAILED;
	}
...
static rt_s zz_test_select_with_statement(struct da_statement *statement, enum da_database_type database_type)
{
	...
	while (RT_TRUE) {
		...
		/* Check VAL1. */
		if (val1) {
			if (RT_UNLIKELY(bindings[0].is_null)) goto end;
			if (RT_UNLIKELY(bindings[0].u.char8.buffer_size != rt_char8_get_size(buffer1))) goto end;
			if (RT_UNLIKELY(!rt_char8_equals(buffer1, rt_char8_get_size(buffer1), val1, rt_char8_get_size(val1)))) goto end;
		} else {
			if (RT_UNLIKELY(!bindings[0].is_null)) goto end;
		}
		...
		if (line == 1 && database_type != DA_DATABASE_TYPE_ORACLE) {
			if (RT_UNLIKELY(bindings[1].is_null)) goto end;
			if (RT_UNLIKELY(rt_char8_get_size(buffer2) != 0)) goto end;
			if (RT_UNLIKELY(bindings[1].u.char8.buffer_size != 0)) goto end;
		} else {
			if (RT_UNLIKELY(!bindings[1].is_null)) goto end;
		}

		/* Check VAL3. */
		if (RT_UNLIKELY(bindings[2].is_null)) goto end;
		if (RT_UNLIKELY(bindings[2].u.n32.value != val3)) goto end;
	}
	...
	if (result_created) {
		if (RT_UNLIKELY(!result.free(&result)))
			ret = RT_FAILED;
	}
...
static rt_s zz_test_select_with_prepared_statement(struct da_statement *statement)
{
	...
	while (RT_TRUE) {
		...
		if (RT_UNLIKELY(bindings[0].is_null)) goto end;
		if (RT_UNLIKELY(bindings[0].u.char8.buffer_size != rt_char8_get_size(buffer1))) goto end;
		if (RT_UNLIKELY(!rt_char8_equals(buffer1, rt_char8_get_size(buffer1), "333", 3))) goto end;
		...
		/* VAL3. */
		if (RT_UNLIKELY(bindings[2].is_null)) goto end;
		if (RT_UNLIKELY(bindings[2].u.n32.value != 4)) goto end;
	}
...
static rt_s zz_test_select_with_connection(struct da_connection *connection, enum da_database_type database_type)
{
	...
	if (prepared_statement_created) {
		if (RT_UNLIKELY(!prepared_statement.free(&prepared_statement)))
			ret = RT_FAILED;
	}
	if (statement_created) {
		if (RT_UNLIKELY(!statement.free(&statement)))
...
static rt_s zz_test_data_source(struct da_data_source *data_source, enum da_database_type database_type)
{
	...
	if (select_connection_created) {
		if (RT_UNLIKELY(!select_connection.free(&select_connection)))
			ret = RT_FAILED;
	}

	if (auto_commit_connection_created) {
		if (RT_UNLIKELY(!auto_commit_connection.free(&auto_commit_connection)))
			ret = RT_FAILED;
	}
...
static rt_s zz_test_driver(struct da_driver *driver, const rt_char8 *host_name, rt_un port, const rt_char8 *database, const rt_char8 *user_name, const rt_char8 *password, enum da_database_type database_type)
{
	...
	if (data_source_created) {
		if (RT_UNLIKELY(!data_source.free(&data_source)))
			ret = RT_FAILED;
	}
...
rt_s zz_test_rda(const rt_char8 *host_name, rt_un port, const rt_char8 *database, const rt_char8 *user_name, const rt_char8 *password, enum da_database_type database_type)
{
	...
	if (driver_created) {
		if (RT_UNLIKELY(!driver.free(&driver)))
			ret = RT_FAILED;
	}
